...
def get_value(username: str, key: str) -> str | None:
    with _conn(username) as db:
        row = db.execute("SELECT value FROM storage WHERE key=?", (key,)).fetchone()
    return row[0] if row else None


def set_value(username: str, key: str, value: str) -> None:
    with _conn(username) as db:
        db.execute(
